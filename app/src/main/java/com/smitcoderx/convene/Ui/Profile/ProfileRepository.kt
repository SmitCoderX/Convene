...
class ProfileRepository @Inject constructor(
    ...
    suspend fun fetchData(id: String): Resource<LoginData?> {
        return try {
            val result = userCollectionRef.document(id).get().await()
            Log.i(TAG, "Add Profile UserData Done --> ${result.data?.entries}")
            Resource.Success(result.toObject(LoginData::class.java))
        } catch (e: Exception) {
            Log.e(TAG, "Fetching Profile UserData: ${e.message}", e.cause)
            Resource.Error(e.message.toString())
        }
    }
